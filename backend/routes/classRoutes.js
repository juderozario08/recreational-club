...
router.get("/classes/member/:memberId", async (req, res) => {
  try {
    const classes = await Class.find({
      "attendees.user": req.params.memberId,
    }).populate("coach attendees.user");
    res.json(classes);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
...
router.post("/classes/:id/users", async (req, res) => {
  try {
    ...
    const isUserAlreadyAdded = existingClass.attendees.some(
...
router.post("/classes/:id/pay", async (req, res) => {
  try {
    ...
    const attendeeIndex = existingClass.attendees.findIndex(
      (attendee) => attendee.user.toString() === userId
    );
    if (attendeeIndex === -1) {
      return res
        .status(404)
        .json({ message: "User not found in class attendees." });
    }
...
router.get("/!user-classes/:userId", async (req, res) => {
  ...
  try {
    ...
    const classesForUser = allClasses.filter(
      (classItem) =>
        !classItem.attendees.some(
          (attendee) => attendee.user._id.toString() === userId
        )
